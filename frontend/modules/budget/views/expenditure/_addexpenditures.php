        ...
        'columns' => [
            [
                ...
                'checkboxOptions' => function($model, $key, $index, $column) use ($id, $year){
                                         $bool = Expenditure::find()->where(['expenditure_object_id' => $model->expenditure_object_id, 'year'=>$year, 'active'=>1])->count();
                                         return ['checked' => $bool,
                                                'onclick'=>'onExpenditure(this.value,'.$year.',this.checked)' //additional
                                                ];
                                     }
            ],
            [
...
function onExpenditure(objectid,year,checked){
    $.ajax({
            type: "POST",
            url: "<?php echo Url::to(['expenditure/addexpenditure']); ?>",
            data: {objectid:objectid,year:year,checked:checked},
